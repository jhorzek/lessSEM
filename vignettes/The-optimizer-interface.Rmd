...
So, you've decided to take a closer look at the optimization routines 
implemented in **lessSEM**. That's awesome! The following document will 
give you a short introduction to the general idea of how the optimizers
were designed. It will not cover the optimizers themselves; here, we recommend
...
The objectives of the optimizers is to minimize the fitting function. In both,
...
penalty: the lasso $p(\pmb\theta)$. This must be an object of class penaltyLASSO
which can be created with lessSEM::penaltyLASSO(). Next comes the differentiable
ridge penalty which must be of class penaltyRidge and can be created with 
lessSEM::penaltyRidge. 
...
both of these into one: the tuningParametersEnet struct. tuningParametersEnet 
...
is printed. If set to -99 you will get the debug output which is horribly convoluted
