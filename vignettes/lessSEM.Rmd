...
penalties: ista and glmnet. Currently, the default is ista, which does not require the 
computation of a Hessian matrix. However, this comes at a price: ista optimization
...
due to the Hessian matrix. Switching to glmnet is done as follows:

```{r, eval = FALSE}
fitLasso <- lasso(lavaanModel = lavaanModel, 
                  regularized = regularized,
                  nLambdas = 10,
                  method = "glmnet", # change the method
                  control = controlGlmnet() # change the control argument
                  )
```
