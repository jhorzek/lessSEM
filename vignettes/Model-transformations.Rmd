...
vignette: >
  %\VignetteIndexEntry{Model-transformations}
...
transformations. This is still an experimental feature, so beware. But if it 
can be used, it is relatively powerful.

## Motivation

In longitudinal SEM, an important question is if parameters stay the same over
time (e.g., measurement invariance). This question can be difficult to answer and
...
However do we know which model to use? There are many procedures to answer this 
...
allow for testing measurement invariance (Belzak & Bauer, 2020, for using regularization
techniques to test measurement invariance). In **regsem**, there is a "diff_lasso"
implementation which regularizes differences between parameters, however this is
not available in **lessSEM**. However, **lessSEM** provides a flexible workaround:
...
```
Note that the differences between the parameters gets smaller with larger $\lambda$ values. 
...
Note that all differences have been zeroed -- that is, a model with measurement invariance
did fit best.
...
is treated as different from the rest: After all, why should $a_1$ serve as baseline and not 
...
be thought of as a regime change model, where there may be multiple regimes (i.e., 
different true parameters for different time points; see also Ou et al., 2019
for regime switching models) and we want to detect if/when the process changes. 

We won't go into the details of how to set up the model here, but you can 
find them in the source of this file 
([e.g., in GitHub](https://github.com/jhorzek/lessSEM/tree/main/vignettes/Model-transformations.Rmd)).
We simulated a data set with 200 individuals measured at 10 time points. The
autoregressive effect $a_t$ changes at $t=5$ from $.6$ to $.2$.
...
And define our transformations as follows:
...
While the result is not perfect, **lessSEM** correctly identfied a change in the
