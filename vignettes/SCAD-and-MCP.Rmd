---
title: "SCAD and MCP: Derivations of the implementation used in lessSEM"
...
\frac{-x^2 + 2\theta\lambda |x| - \lambda^2}{2(\theta -1)} & \text{if } \lambda < |x| \leq \lambda\theta\\
(\theta + 1) \lambda^2/2 & \text{if } |x| \geq \theta\lambda\\
\end{cases}$$

with $\theta > 2$ and $\lambda > 0$. For the proximal operator we are searching a solution for the function
$\hat x  = \arg\min_x \frac 12 (x-u)^2 + \frac 1t p(x)$. The idea of Gong et al. (2013) is to minimze this function for each of the regions mentioned above and to compare these minima to find the global minimum.

## Assume: $|x| \leq \theta$

Assuming that the solution is in the region $|x| \leq \theta$, the scad is identical to the lasso. It follows:

$$\hat x = \text{sign} (u)\max(0,|u|-\lambda / t)$$
If we also take the border $|x| \leq \theta$ into account, it follows:

1. If $x > 0$: $\hat x = \min(\lambda, \text{sign} (u)\max(0,|u|-\lambda / t))$
2. If $x < 0$: $\hat x = \max(-\lambda, \text{sign} (u)\max(0,|u|-\lambda / t))$
...
$$
\hat x = \begin{cases}
\min(\lambda\theta, \max(\lambda, \frac{u}{v} - \frac{\theta\lambda}{t(\theta-1)v}) & \text{if } x > 0\\
\max(-\lambda\theta, \min(-\lambda, \frac{u}{v} + \frac{\theta\lambda}{t(\theta-1)v}) & \text{if } x < 0\\
\end{cases}
$$
...
Now we combine the differentiation of the penalty with the differentiation of $\frac 12 (x-u)^2$ and set to 0:
...
### Assume that the solution is given by $x > 0$. 
...
In this case, the differentiation with respect to $x$ is well defined. We get $\frac{\partial}{\partial x}p(x) = 0$ and $x = u$ as minimum of the function. Respecting the borders:
