...
vignette: >
  %\VignetteIndexEntry{Multi-Group-SEM}
...
models at once which are then combined into a multi-group model. In the following, we will look at a two-group model. 
...
```{r}
coef(fit, criterion = "AIC")[,c("l1_Pasteur", "l1_delta")]
```
...
fixed to specific values. Unfortunately, **lavaan** does not allow us to set
