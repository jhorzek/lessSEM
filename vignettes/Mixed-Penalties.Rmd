...
Mixed penalties are currently supported for:
...
mp <- lavaanModel |>
  ...
  addScad(regularized = c("r1", "r2", "r3"), 
          ...
          thetas = 3.7) 
```
The functions to add penalties are called
...
mp <- lavaanModel |>
  mixedPenalty() |>
  # We will add two lasso penalties, identical to the example given above.
  # Because glmnet only supports elasticNet penalties, we have to set alpha to
  # 1 for the elasticNet to get a lasso penalty. Setting alpha to zero results in
  # ridge regularization.
  addElasticNet(regularized = c("c2", "c3", "c4"), 
                lambdas = seq(0,1,.1),
                alphas = 1) |>
  addElasticNet(regularized = c("r1", "r2", "r3"), 
                lambdas = seq(0,1,.2),
                alphas = 1) 
```
