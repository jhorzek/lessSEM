test_that("testing cross-validation for scad", {
  library(regsem)
  library(lessSEM)
  set.seed(123)
  N <- 50
  ...
  lambdas <- seq(0,1,.1)
  thetas <- c(2.3,3.6)
  rsem <- lessSEM::scad(lavaanModel = modelFit, 
  ...
  cv <- cvScad(lavaanModel = modelFit, 
              regularized = regularizedLavaan,
              lambdas = lambdas,
              thetas = thetas,
  ...
  cv3 <- cvScad(lavaanModel = modelFit, 
               regularized = regularizedLavaan,
               lambdas = lambdas,
               thetas = thetas,
               k = 3)
  ...
  subsetCappedL1 <- scad(lavaanModel = modelFit, 
                        regularized = regularizedLavaan,
                        lambdas = lambdas,
                        thetas = thetas,
