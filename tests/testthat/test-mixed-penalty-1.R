test_that("testing mixed penalty - 1", {
  testthat::skip_on_cran()

library(lessSEM)
set.seed(123)

dataset <- simulateExampleData()

lavaanSyntax <- "
f =~ l1*y1 + l2*y2 + l3*y3 + l4*y4 + l5*y5 +
     l6*y6 + l7*y7 + l8*y8 + l9*y9 + l10*y10 +
     l11*y11 + l12*y12 + l13*y13 + l14*y14 + l15*y15
f ~~ 1*f
"

lavaanModel <- lavaan::sem(lavaanSyntax,
                           data = dataset,
                           meanstructure = TRUE,
                           std.lv = TRUE)


tuningParameters <- expand.grid(lambda1 = seq(0,1,length.out = 3),
                                lambda2 = seq(0,1,length.out = 3))

penaltyFunctionArguments <- list(
  eps = 1e-8,
  regularizedLasso1 = paste0("l", 6:10),
  regularizedLasso2 = paste0("l", 11:15)
)

# Now, it is time to specify our custom penalty function:

smoothLASSO <- function(
    # here are our three arguments:
  parameters,
  tuningParameters,
  penaltyFunctionArguments
){
  lambda1 <- tuningParameters$lambda1 
  lambda2 <- tuningParameters$lambda2
  
  eps <- penaltyFunctionArguments$eps
  regularizedLasso1 <- penaltyFunctionArguments$regularizedLasso1
  regularizedLasso2 <- penaltyFunctionArguments$regularizedLasso2
  
  penaltyLasso1 <- lambda1*sum(sqrt(parameters[regularizedLasso1]^2 + eps))
  penaltyLasso2 <- lambda2*sum(sqrt(parameters[regularizedLasso2]^2 + eps))
  
  return(penaltyLasso1 + penaltyLasso2)
}

#### Now we are ready to optimize! ####
regsemApprox <- lessSEM:::.regularizeSEMWithCustomPenaltyRsolnp(lavaanModel = lavaanModel,
                                                                individualPenaltyFunction = smoothLASSO,
                                                                tuningParameters = tuningParameters,
                                                                penaltyFunctionArguments = penaltyFunctionArguments)

# let's compare the results to an exact optimization:

exact <- lavaanModel |>
  mixedPenalty() |>
  addLasso(regularized = paste0("l", 6:10),
           lambdas = seq(0,1,length.out = 3)) |>
  addLasso(regularized = paste0("l", 11:15),
           lambdas = seq(0,1,length.out = 3)) |>
  fit()


testthat::expect_true(
  all(abs(exact@parameters[,exact@parameterLabels] -
            regsemApprox@parameters[,exact@parameterLabels]) < .1)
)

testthat::expect_lt(
  max(abs(regsemApprox@fits$regM2LL - 
            exact@fits$regM2LL)/regsemApprox@fits$regM2LL),  .01) # differ by less than 1 percent

})