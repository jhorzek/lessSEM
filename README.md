...
(e.g., lasso is called with the lasso() function). Smooth functions are
...
**These are also available for other packages.** There are two ways to
implement them:

1.  using the R interface: All general purpose implementations of the
    functions are called with prefix “gp” (e.g., gpLasso, gpScad, …).
    More information and examples can be found in the documentation of
    these functions (e.g., ?lessSEM::gpLasso, ?lessSEM::gpAdaptiveLasso,
    ?lessSEM::gpElasticNet). The interface is similar to the optim
    optimizers in R.
2.  All optimizers are implemented as C++ header-only files in lessSEM.
