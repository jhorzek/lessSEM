...
#' regularizeSEMWithCustomPenaltyOptim
#' 
#' Optimize a SEM with custom penalty function using the optim optimizers (see ?optim). This optimizer is the default in regsem (see ?regsem::cv_regsem).
#' 
#' @param lavaanModel model of class lavaan 
#' @param individualPenaltyFunction penalty function which takes the current parameter values as first argument, the tuning parameters as second, and the penaltyFunctionArguments as third argument and 
#' returns a single value - the value of the penalty function for a single person. If the true penalty function is non-differentiable (e.g., lasso) a smooth
...
#' the next iteration? Interestingly, this can result in worse parameter estimates...
#' @param saveHessian should the Hessian of the optimizer be saved? This will take a lot of space!
#' @param method which optim method should be used?
#' @param control option to set parameters of the optimizer; see ?optim
#' @examples 
#' library(aCV4SEM)
#' 
#' # Identical to regsem, aCV4SEM builds on the lavaan
...
#' 
#' #### Now we are ready to optimize! ####
#' regsemApprox <- regularizeSEMWithCustomPenaltyOptim(lavaanModel = lavaanModel,
#'                                                     individualPenaltyFunction = smoothLASSO, 
#'                                                     individualPenaltyFunctionGradient = smoothLASSOGradient,
#'                                                     tuningParameters = tuningParameters,
#'                                                     penaltyFunctionArguments = penaltyFunctionArguments)
...
#' coef(aCV)
#' @export
regularizeSEMWithCustomPenaltyOptim <- function(lavaanModel, 
                                                individualPenaltyFunction,
                                                individualPenaltyFunctionGradient = NULL,
                                                tuningParameters,
                                                penaltyFunctionArguments,
                                                ...
                                                method = "BFGS",
                                                control = list("trace" = 0)){
  warning("Using Optim does, in our experience, fail relatively often.")
  ...
  fitfun <- function(parameters, 
    ...
    fSEM <- try(aCV4SEM:::fit(SEM), silent = TRUE)
    if(is(fSEM, "try-error") || !is.finite(fSEM$m2LL)){
      return(99999999999999999)
    }
    ...
    return(fSEM$m2LL + penalty)
  }
  
  gradfun <- function(parameters, 
                      SEM, 
                      sampleSize,
                      individualPenaltyFunction, 
                      individualPenaltyFunctionGradient,
    ...
    gradients[] <- 5
    gr <- try(aCV4SEM:::getGradients(SEM = SEM, raw = TRUE), silent = TRUE)
    if(is(gr, "try-error") || any(!is.finite(gr))){
      return(gradients)
    }
    
    gr <- gr + 
      (sampleSize*individualPenaltyFunctionGradient(parameters, 
                                                    tuningParameters, 
                                                    penaltyFunctionArguments))[names(gr)]
    return(gr)
  }
  ...
  for(i in 1:nrow(tuningParameters)){
    ...
    result <- optim(par = parametersInit, fn = fitfun, 
                    gr = gradfun,
                    SEM = SEM, 
                    sampleSize = sampleSize,
                    individualPenaltyFunction = individualPenaltyFunction, 
    ...
    # The optimizer builds an approximation of the Hessian which
    # will be better if more iterations have to be done until convergence
    if(carryOverParameters) parametersInit <- result$par
    
    SEM <- try(aCV4SEM:::setParameters(SEM = SEM, labels = names(result$par), 
                                       values = result$par, raw = TRUE), 
               silent = TRUE)
    SEMFit <- try(aCV4SEM:::fit(SEM), silent = TRUE)
    if(is(SEMFit, "try-error")) {
      if(i-1 <= 0) stop("Could not fit model.")
      parametersInit <- parameterEstimates[i-1, names(parameters)]
      fits$convergence[i] <- FALSE
      next
    }
    
    parameterEstimates[i, names(parameters)] <- aCV4SEM:::getParameters(SEM, 
                                                                        raw = FALSE)[names(parameters)]
    fits$m2LL[i] <- SEM$m2LL
...
#' regularizeSEMWithCustomPenaltyNlminb
#' 
#' Optimize a SEM with custom penalty function using the optim optimizers (see ?optim). This optimizer is the default in regsem (see ?regsem::cv_regsem).
#' 
#' @param lavaanModel model of class lavaan 
...
#' 
#' #### Now we are ready to optimize! ####
#' regsemApprox <- regularizeSEMWithCustomPenaltyNlminb(lavaanModel = lavaanModel,
#'                                                     individualPenaltyFunction = smoothLASSO, 
#'                                                     individualPenaltyFunctionGradient = smoothLASSOGradient,
#'                                                     tuningParameters = tuningParameters,
#'                                                     penaltyFunctionArguments = penaltyFunctionArguments)
...
#' coef(aCV)
#' @export
regularizeSEMWithCustomPenaltyNlminb <- function(lavaanModel, 
                                                 individualPenaltyFunction,
                                                 individualPenaltyFunctionGradient = NULL,
                                                 tuningParameters,
                                                 penaltyFunctionArguments,
  ...
  for(i in 1:nrow(tuningParameters)){
    ...
    result <- nlminb(start = parametersInit, 
                     objective = fitfun, 
                     gradient = gradfun,
                     SEM = SEM, 
                     sampleSize = sampleSize,
    ...
    parameterEstimates[i, names(parameters)] <- aCV4SEM:::getParameters(SEM, 
                                                                        raw = FALSE)[names(parameters)]
    fits$m2LL[i] <- SEM$m2LL
    fits$regM2LL[i] <- result$objective
    fits$convergence[i] <- result$convergence
